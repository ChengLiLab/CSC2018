...
SI12D_XEN_tsne <- read.csv("StageI/AGG_SI12D_XEN_mean0.5_dim7.csv", stringsAsFactors = F, row.names = 1)
set.seed(1011)

### set colors for time points ----
SI12D_XEN_tsne$Batch <- factor(SI12D_XEN_tsne$Batch,
                               levels = c("XEN","SI12D"),ordered = TRUE)
myColors <- c("#F5A623","#0365BF")
names(myColors) <- c("XEN","SI12D")
colScale <- scale_colour_manual(name = c("XEN","SI12D"),values = myColors)

### plot t-SNE with time point information ----
p <- ggplot() +
  geom_point(data = SI12D_XEN_tsne, aes(x=data_dim_1, y=data_dim_2, color = Batch)) +
  colScale +
  theme_classic() +
  # ggtitle("SI12D_XEN") +
  labs(x = " ") + 
...
# save figure
tiff(file=paste0("StageI/tSNE_SI12D_XEN_Batch.tiff"), width = 7, height = 7, units = 'in', res = 300, compression = "none")
print(p)
dev.off()

### plot t-SNE with genes expression information ----
data <- SI12D_XEN_tsne[,c(2,3,4,6,10)]
...
for (gene in genes) {
  ...
  tiff(file=paste0("StageI/SI12D_XEN_tSNE_",gene,"_mean",0.5,"_dim",7,".tiff"), width = 7, height = 7, units = 'in', res = 300, compression = "none")
  print(p)
  dev.off()
}